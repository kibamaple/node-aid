...
const {ENV} = process.env,
    ...
    {parse} = Querystring,
    {compile} = Handlebars,
    render = compile('<form id="alipay" name="alipay" action="'+URL+'?charset={{params.charset}}" method="post" accept-charset="{{params.charset}}" onsubmit="document.charset=\'{{params.charset}}\';">\n{{#each params as |value key|}}\n<input name="{{key}}" type="hidden" value="{{value}}" />\n{{/each}}\n</form>\n<script>document.forms[\'alipay\'].submit();</script>'),
